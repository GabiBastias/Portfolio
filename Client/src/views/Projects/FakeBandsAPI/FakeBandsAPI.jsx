...
// import FakeBandAPIProjectIMG from '../../../assets/img/FakeBandAPI.png'
...
const FakeBandAPI = ({ language }) => {
    ...
    const handleObjectives = (event) => {
        ...
        if (event.target.id === 'btnInfoFBA') {
            if(divListFBA.classList.value === "showOrNot"){
                divListFBA.classList.value = "divListOpen";
    ...
    return(
        <article className={styles.articleFakeBandsAPI}>
            <img className={styles.backgroundIMG} src="" alt="backgroundIMG" />
            <div className={styles.divLeft}>
                <section className='showOrNot' id='divListFBA'>
                    <h2 className={styles.h2Objectives}>Back-end Developer</h2>
                    {
                        ...
                        <div className={styles.divFBA}>
                            ...
                            <p className={styles.infoTecFBA}>JWT - Supertest - Nodemailer - Bcrypt.</p>
                            <br />
                ...
                <section className='showOrNot' id='divExperienceFBA'>
                    <h2 className={styles.h2Objectives}>{language === "Spanish" ? "Experiencia" : "Experience"}</h2>
                    {
                        language === "Spanish" && 
                        <div>
                            <p className={styles.pFBA}>En este proyecto, asumí el rol de liderazgo en el desarrollo de la parte del servidor, colaborando con éxito con dos colegas. Nos embarcamos en la tarea de trabajar con tecnologías clave, como JWT para la autenticación, Jest y Supertest para la prueba de rutas y modelos de base de datos, mailer para el envío de correos electrónicos a los usuarios, y mongoose como ORM, entre otras.</p>

                            <p className={styles.pFBA}>Creamos una API-REST para acceder de manera eficiente a la base de datos, permitiendo la manipulación precisa de datos relacionados con usuarios, cursos y otra información relevante para el desbloqueo de funcionalidades. Los resultados obtenidos fueron óptimos desde el lado del servidor, garantizando un acceso preciso y seguro a la información y las rutas.</p>
                            
                            <p className={styles.pFBA}>Mi liderazgo en la implementación de la parte del servidor fue esencial para el éxito del proyecto, asegurando una colaboración efectiva entre el equipo y una implementación fluida de las tecnologías seleccionadas.</p>
                        </div>
                    }
                    {
                        language === "English" && 
                        <div>
                            <p className={styles.pFBA}>In this project, I took on the leadership role in the development of the server side, successfully collaborating with two colleagues. We embarked on the task of working with key technologies, such as JWT for authentication, Jest and Supertest for testing routes and database models, mailer for sending emails to users, and mongoose as ORM, among others.</p>

                            <p className={styles.pFBA}>We created a REST-API to efficiently access the database, allowing precise manipulation of data related to users, courses and other information relevant to unlocking functionalities. The results obtained were optimal from the server side, guaranteeing precise and secure access to information and routes.</p>

                            <p className={styles.pFBA}>My leadership in the implementation of the server side was essential to the success of the project, ensuring effective collaboration between the team and a smooth implementation of the selected technologies.</p>
                        </div>
                    }
                </section>
            </div>
            <section className={styles.sectionFakeBandsAPI}>
                <h2 className={styles.h1FakeBandsAPI}>The Fourth Element</h2>
                <div className={styles.divImgP}>
                    <a href="https://the-fourth-element-client.vercel.app/" target="_blank" rel="noreferrer noopener" className={styles.link}>
                        <img className={styles.FakeBandAPIProjectIMG} src="" alt="FakeBandAPI" />
                        {
                            language === "Spanish" &&
                            <p className={styles.pInfoFakeBandsAPI}>Aplicación web que contiene cursos destinado a deportistas de alto rendimiento; este projecto fue reportado directamente a un experto en el mundo del Coaching y Biología molecular.</p>
                        }
                        {
                            language === "English" &&
                            <p className={styles.pInfoFakeBandsAPI}>Web application to access courses for high-performance athletes, for a well-known client in the world of coaching and molecular biology.</p>
                        }
                    </a>
                </div>
                <div>
